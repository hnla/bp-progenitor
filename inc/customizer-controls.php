 ...
 * @since 1.0.0
 */
class BP_Progenitor_Nav_Customize_Control extends WP_Customize_Control {
	/**
	 * @var string
	 */
	public $type = '';
	...
	public function render_content() {
		...
		$setting  = "bp_progenitor_appearance[{$this->type}_nav_order]";
		$item_nav = array();
		...
		if ( 'group' === $this->type ) {
			...
			if ( ! empty( $random['groups'] ) ) {
				...
				$nav      = new BP_Progenitor_Customizer_Group_Nav( $group->id );
		...
		// It's a user!
			...
			$item_nav = bp_progenitor_member_customizer_nav();
		}
		?>
