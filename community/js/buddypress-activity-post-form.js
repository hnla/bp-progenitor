/* global bp, BP_Progenitor, _, Backbone */
...
( function( exports, $ ) {
	...
	// Bail if not set
	if ( typeof bp.Progenitor.Activity === 'undefined' || typeof BP_Progenitor === 'undefined' ) {
		return;
	}
	 ...
	 * @type {Object}
	 */
	bp.Progenitor.Activity.postForm = {
		start: function() {
		...
		postFormView: function() {
			...
			// Display it
			postForm.inject( '#bp-progenitor-activity-form' );
		}
	};
		...
		sync: function( method, model, options ) {
			...
			if ( 'read' === method ) {
				...
				options.data = _.extend( options.data || {}, {
					action: 'bp_Progenitor_get_activity_objects'
				} );
	...
	bp.Views.WhatsNew = bp.View.extend( {
		...
		attributes: {
			...
			cols         : '50',
			rows         : '4',
			placeholder  : BP_Progenitor.activity.strings.whatsnewPlaceholder,
			'aria-label' : BP_Progenitor.activity.strings.whatsnewLabel
		},
		...
		adjustContent: function() {
			...
			// Check for mention
			var	mention = bp.Progenitor.getLinkParams( null, 'r' ) || null;
	...
	bp.Views.WhatsNewPostIn = bp.View.extend( {
		...
		attributes: {
			...
			'aria-label' : BP_Progenitor.activity.strings.whatsnewpostinLabel
		},
		...
		autoComplete: function() {
			...
			this.collection.fetch( {
				data: {
					...
					search : search,
					nonce  : BP_Progenitor.nonces.activity
				},
	...
	bp.Views.FormAvatar = bp.View.extend( {
		...
		initialize: function() {
			this.model = new Backbone.Model( _.pick( BP_Progenitor.activity.params, [
				'user_id',
				'avatar_url',
				'avatar_width',
				'avatar_height',
	...
	bp.Views.FormTarget = bp.View.extend( {
		...
		initialize: function() {
			var select = new bp.Views.WhatsNewPostIn( { filters: BP_Progenitor.activity.params.objects } );
			this.views.add( select );

			select.model.on( 'change', this.attachAutocomplete, this );
			bp.Progenitor.Activity.postForm.ActivityObjects.on( 'change:selected', this.postIn, this );
		},

		attachAutocomplete: function( model ) {
			if ( 0 !== bp.Progenitor.Activity.postForm.ActivityObjects.models.length ) {
				bp.Progenitor.Activity.postForm.ActivityObjects.reset();
			}
			...
			if ( 'profile' !== model.get( 'selected') ) {
				this.views.add( new bp.Views.AutoComplete( {
					collection:   bp.Progenitor.Activity.postForm.ActivityObjects,
	...
	bp.Views.PostForm = bp.View.extend( {
		...
		displayFull: function( event ) {
			...
			if ( true === BP_Progenitor.activity.params.backcompat ) {
				this.views.add( new bp.Views.FormOptions( { model: this.model } ) );
			}

			// Attach buttons
			if ( ! _.isUndefined( BP_Progenitor.activity.params.buttons ) ) {
				// Global
				bp.Progenitor.Activity.postForm.buttons.set( BP_Progenitor.activity.params.buttons );
				this.views.add( new bp.Views.FormButtons( { collection: bp.Progenitor.Activity.postForm.buttons, model: this.model } ) );
			}

			// Select box for the object
			if ( ! _.isUndefined( BP_Progenitor.activity.params.objects ) && 1 < _.keys( BP_Progenitor.activity.params.objects ).length ) {
		...
		postUpdate: function( event ) {
			...
			var data = {
				'_wpnonce_post_update': BP_Progenitor.activity.params.post_nonce
			};
			...
			bp.ajax.post( 'post_update', _.extend( data, this.model.attributes ) ).done( function( response ) {
				var scope     = bp.Progenitor.getStorage( 'bp-activity', 'scope' ),
					prepended = false;
				...
				if ( ! response.is_directory ||
					...
					if ( ! $( '#activity-' + response.id  ).length ) {
						bp.Progenitor.inject( '#activity-stream', response.activity, 'prepend' );
						prepended = true;
					}
				}
	...
	bp.Progenitor.Activity.postForm.start();
