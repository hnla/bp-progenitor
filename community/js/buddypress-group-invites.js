/* global wp, bp, BP_Progenitor, _, Backbone */
window.wp = window.wp || {};
window.bp = window.bp || {};
...
( function( exports, $ ) {
	...
	bp.Progenitor.GroupInvites = {
		...
		start: function() {
			...
			this.setupNav();
			this.setupLoops();
			this.displayFeedback( BP_Progenitor.group_invites.loading, 'loading' );
		...
		setupNav: function() {
			...
			_.each( BP_Progenitor.group_invites.nav, function( item, index ) {
				if ( ! _.isObject( item ) ) {
					return;
				}
		...
		loadConfirmView: function() {
			...
			this.displayFeedback( BP_Progenitor.group_invites.invites_form, 'help' );
	...
	bp.Collections.Users = Backbone.Collection.extend( {
		...
		initialize : function() {
			this.options = { page: 1, total_page: 0, group_id: BP_Progenitor.group_invites.group_id };
		},
		...
		sync: function( method, model, options ) {
			...
			// Add generic nonce
			options.data.nonce = BP_Progenitor.nonces.groups;
			...
			if ( 'create' === method ) {
				options.data = _.extend( options.data, {
					...
					_wpnonce : BP_Progenitor.group_invites.nonces.send_invites
				} );
	...
	bp.Progenitor.GroupInvites.View = bp.Backbone.View.extend( {
	...
	// Feedback view
	bp.Views.Feedback = bp.Progenitor.GroupInvites.View.extend( {
		tagName   : 'div',
	...
	bp.Views.invitesNav = bp.Progenitor.GroupInvites.View.extend( {
		...
		showHideNavItem: function( item ) {
			...
			if ( ! _.isBoolean( isRendered ) ) {
				item.set( 'invites_count', bp.Progenitor.GroupInvites.invites.length );
				this.outputNav( item );
			}
		},
	...
	bp.Views.invitesNavItem = bp.Progenitor.GroupInvites.View.extend( {
		tagName  : 'li',
		...
		initialize: function() {
			...
			bp.Progenitor.GroupInvites.invites.on( 'add', this.updateCount, this );
			bp.Progenitor.GroupInvites.invites.on( 'remove', this.updateCount, this );
		},
	...
	bp.Views.Pagination = bp.Progenitor.GroupInvites.View.extend( {
		tagName   : 'li',
		className : 'last',
		template  :  bp.template( 'bp-invites-paginate' )
	} );

	bp.Views.inviteFilters = bp.Progenitor.GroupInvites.View.extend( {
		tagName: 'div',
		...
		filterUsers: function() {
			bp.Progenitor.GroupInvites.displayFeedback( BP_Progenitor.group_invites.loading, 'loading' );
		...
		usersFiltered: function() {
			bp.Progenitor.GroupInvites.removeFeedback();
		},

		usersFilterError: function( collection, response ) {
			bp.Progenitor.GroupInvites.displayFeedback( response.feedback, 'error' );
		},
	...
	bp.Views.inviteUsers = bp.Progenitor.GroupInvites.View.extend( {
		tagName   : 'ul',
		...
		usersFetched: function( collection, response ) {
			bp.Progenitor.GroupInvites.displayFeedback( response.feedback, 'help' );
		},

		usersFetchError: function( collection, response ) {
			var type = response.type || 'help';

			bp.Progenitor.GroupInvites.displayFeedback( response.feedback, type );
		},
	...
	bp.Views.inviteUser = bp.Progenitor.GroupInvites.View.extend( {
		tagName  : 'li',
		...
		initialize: function() {
			var invite = bp.Progenitor.GroupInvites.invites.get( this.model.get( 'id' ) );
		...
		toggleUser: function( event ) {
			...
			if ( false === selected ) {
				...
				if ( ! bp.Progenitor.GroupInvites.invites.length  ) {
					bp.Progenitor.GroupInvites.invites.reset();
				}
			}
		},
		...
		inviteRemoved: function( response ) {
			...
			if ( false === response.has_invites ) {
				bp.Progenitor.GroupInvites.displayFeedback( response.feedback, 'success' );

				// Hide the invited nav
				bp.Progenitor.GroupInvites.navItems.get( 'invited' ).set( { active: 0, hide: 1 } );
			}
		},
	...
	bp.Views.invitesEditor = bp.Progenitor.GroupInvites.View.extend( {
		tagName : 'div',
		...
		invitesSent: function( response ) {
			...
			if ( 1 === bp.Progenitor.GroupInvites.navItems.get( 'invited' ).get( 'hide' ) && ! BP_Progenitor.group_invites.is_group_create ) {
				bp.Progenitor.GroupInvites.navItems.get( 'invited' ).set( { active: 0, hide: 0 } );
			}
		},
		...
		invitesError: function( response ) {
			...
			if ( ! _.isUndefined( response.users ) ) {
				// Display the pending invites
				if ( 1 === bp.Progenitor.GroupInvites.navItems.get( 'invited' ).get( 'hide' ) && response.users.length < this.collection.length ) {
					bp.Progenitor.GroupInvites.navItems.get( 'invited' ).set( { active: 0, hide: 0 } );
				}
		...
		cleanViews: function() {
			...
			bp.Progenitor.GroupInvites.displayFeedback( BP_Progenitor.group_invites.invites_form_reset, 'help' );
		}
	} );

	bp.Views.invitesForm = bp.Progenitor.GroupInvites.View.extend( {
		tagName  : 'div',
	...
	bp.Views.selectedUser = bp.Progenitor.GroupInvites.View.extend( {
		tagName  : 'li',
	...
	bp.Progenitor.GroupInvites.start();
