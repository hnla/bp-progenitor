...
( function( exports, $ ) {
	...
	bp.Progenitor = {
		...
		setupGlobals: function() {
			...
			// Object Globals
			this.objects                = $.map( BP_Progenitor.objects, function( value ) { return value; } );
			this.objectNavParent        = BP_Progenitor.object_nav_parent;
			this.time_since             = BP_Progenitor.time_since;
		...
		prepareDocument: function() {
			...
			if ( BP_Progenitor.warnings && 'undefined' !== typeof console && console.warn ) {
				$.each( BP_Progenitor.warnings, function( w, warning ) {
		...
		ajax: function( post_data, object ) {
			...
			var postData = $.extend( {}, bp.Progenitor.getStorage( 'bp-' + object ), { nonce: BP_Progenitor.nonces[object] }, post_data );

			if ( undefined !== BP_Progenitor.customizer_settings ) {
				postData.customized = BP_Progenitor.customizer_settings;
			}

			this.ajax_request = $.post( BP_Progenitor.ajaxurl, postData, 'json' );
		...
		setHeartBeat: function() {
			if ( typeof BP_Progenitor.pulse === 'undefined' || ! this.heartbeat ) {
				return;
			}

			this.heartbeat.interval( Number( BP_Progenitor.pulse ) );
