...
( function( exports, $ ) {
	...
	bp.Progenitor.Activity = {
		...
		setupGlobals: function() {
			...
			// Init mentions count
			this.mentions_count = Number( $( bp.Progenitor.objectNavParent + ' [data-bp-scope="mentions"]' ).find( 'a span' ).html() ) || 0;
		...
		addListeners: function() {
			...
			// Activity actions
			$( '#buddypress [data-bp-list="activity"]' ).on( 'click', '.activity-item', bp.Progenitor, this.activityActions );
		...
		heartbeatSend: function( event, data ) {
			...
			$.extend( data, { bp_heartbeat: bp.Progenitor.getStorage( 'bp-activity' ) } );

			// Update all displayed time
			$.each( $( '#buddypress time' ), function( t, time ) {
				if ( $( time ).data( 'bp-timestamp' ) ) {
					$( time ).html( bp.Progenitor.updateTimeSince( Number( $( time ).data( 'bp-timestamp' ) ) ) );
				}
			} );
		},
		...
		heartbeatTick: function( event, data ) {
			var newest_activities_count, newest_activities, objects = bp.Progenitor.objects,
				scope = bp.Progenitor.getStorage( 'bp-activity', 'scope' ), self = this;
			...
			if ( 'all' === scope ) {
				 ...
				 */
				 $( bp.Progenitor.objectNavParent + ' [data-bp-scope="all"]' ).find( 'a span' ).html( newest_activities_count );
			...
			$.each( objects, function( o, object ) {
				if ( undefined !== self.heartbeat_data.highlights[ object ] && self.heartbeat_data.highlights[ object ].length ) {
					...
					$( bp.Progenitor.objectNavParent + ' [data-bp-scope="' + object + '"]' ).find( 'a span' ).html( Number( self.heartbeat_data.highlights[ object ].length ) + count );
				}
			} );
			...
			// Otherwise add it
				...
				$( '#buddypress [data-bp-list="activity"]' ).prepend( '<li class="load-newest"><a href="#newest">' + BP_Progenitor.newest + ' (' + newest_activities_count + ')</a></li>' );
			}
		...
		injectActivities: function( event ) {
			var store = bp.Progenitor.getStorage( 'bp-activity' ),
			...
			if ( $( event.currentTarget ).hasClass( 'load-newest' ) ) {
				...
				if ( 'all' === scope ) {
					$( bp.Progenitor.objectNavParent + ' [data-bp-scope="all"]' ).find( 'a span' ).html( '' );
				}

				// Specific to mentions
				if ( 'mentions' === scope ) {
					// Now mentions are displayed, remove the user_metas
					bp.Progenitor.ajax( { action: 'activity_clear_new_mentions' }, 'activity' );
					this.mentions_count = 0;
				}

				// Activities are now displayed, clear the newest count for the scope
				$( bp.Progenitor.objectNavParent + ' [data-bp-scope="' + scope + '"]' ).find( 'a span' ).html( '' );
			...
			comments.each( function( c, comment ) {
				...
				comment_items.each( function( i, item ) {
					if ( i < comment_items.length - 5 ) {
						...
						if ( ! i ) {
							$( item ).before( '<li class="show-all"><a href="#' + activity_item.prop( 'id' ) + '/show-all/">' + BP_Progenitor.show_x_comments.replace( '%d', comment_count ) + '</a></li>' );
						}
					}
				} );
		...
		activityActions: function( event ) {
			...
			if ( target.hasClass( 'delete-activity' ) || target.hasClass( 'acomment-delete' ) || target.hasClass( 'spam-activity' ) || target.hasClass( 'spam-activity-comment' ) ) {
				...
				if ( undefined !== BP_Progenitor.confirm && false === window.confirm( BP_Progenitor.confirm ) ) {
					return false;
				}
					...
					if ( false === response.success ) {
						...
						if ( activity_comment_id ) {
							...
							if ( show_all_a.length ) {
								show_all_a.html( BP_Progenitor.show_x_comments.replace( '%d', comment_count ) );
							}
