 ...
 */
function bp_progenitor_xprofile_register_scripts( $scripts = array() ) {
	if ( ! isset( $scripts['bp-progenitor'] ) ) {
		return $scripts;
	}

	return array_merge( $scripts, array(
		'bp-progenitor-xprofile' => array(
 ...
 * @since 1.0.0
 */
function bp_progenitor_xprofile_enqueue_scripts() {
	if ( ! bp_is_user_profile_edit() && ! bp_is_register_page() ) {
		return;
	}

	wp_enqueue_script( 'bp-progenitor-xprofile' );
}
