 ...
 * @since 1.0.0
 */
function bp_progenitor_ajax_object_template_loader() {
	...
	// Nonce check!
	if ( empty( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'bp_progenitor_' . $object ) ) {
		wp_send_json_error();
	}
 ...
 *     - 'bp_progenitor_ajax_messages_send_message' is the hooked function to the AJAX action.
 *     - 'nopriv' indicates whether the AJAX action is allowed for logged-out users.
 * }
 * @return array
 */
function bp_progenitor_register_ajax_actions( $ajax_actions = array() ) {
