 ...
 */
function bp_progenitor_hook( $pieces = array() ) {
	if ( ! $pieces ) {
		return;
	}
 ...
 */
function bp_progenitor_plugin_hook( $suffix = '' ) {
	if ( ! $suffix ) {
		return;
	}

	bp_progenitor_hook( array(
		'bp',
		'template',
		$suffix,
	) );
 ...
 * Fire friend hooks
 *
 * @todo Move this into bp-progenitor/includes/friends/template-tags.php
 ...
 * @param string The suffix of the hook.
 */
function bp_progenitor_friend_hook( $suffix = '' ) {
	if ( ! $suffix ) {
		return;
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_template_message_classes() {
	$classes = array( 'bp-feedback', 'bp-messages' );

	if ( ! empty( bp_progenitor()->template_message['message'] ) ) {
		$classes[] = 'bp-template-notice';
	}

	$classes[] = bp_progenitor_get_template_message_type();
	 ...
	 */
	function bp_progenitor_get_template_message_type() {
		$bp_progenitor = bp_progenitor();
		$type       = 'error';

		if ( ! empty( $bp_progenitor->template_message['type'] ) ) {
			$type = $bp_progenitor->template_message['type'];
		} elseif ( ! empty( $bp_progenitor->user_feedback['type'] ) ) {
			$type = $bp_progenitor->user_feedback['type'];
		}
 ...
 */
function bp_progenitor_has_template_message() {
	$bp_progenitor = bp_progenitor();

	if ( empty( $bp_progenitor->template_message['message'] ) && empty( $bp_progenitor->user_feedback ) ) {
		return false;
	}
 ...
 * BP template notices - Progenitor user_feedback key needs review.
	*
 *
 * @since 1.0.0
 *
 ...
 */
function bp_progenitor_has_dismiss_button() {
	$bp_progenitor = bp_progenitor();

	// BP template notices - set 'dismiss' true for a type in `bp_progenitor_template_notices()`
	if ( ! empty( $bp_progenitor->template_message['message'] ) && true === $bp_progenitor->template_message['dismiss'] ) {
		return true;
	}

	// Test for isset as value may be bool or string i.e 'clear'
	if ( isset( $bp_progenitor->user_feedback['dismiss'] ) ) {
		return true;
	}
 ...
 * 'clear' is tested for & used to remove cookies, if set, in buddypress-progenitor.js.
 * Currently template_notices(BP) will take $type = 'clear' if button set to true.
 *
 * @since 1.0.0
 */
function bp_progenitor_dismiss_button_type() {
	$bp_progenitor = bp_progenitor();
	$type = 'clear';

	if ( ! empty( $bp_progenitor->user_feedback['dismiss'] ) ) {
		$type = $bp_progenitor->user_feedback['dismiss'];
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_template_message() {
	echo bp_progenitor_get_template_message();
}
	 ...
	 */
	function bp_progenitor_get_template_message() {
		$bp_progenitor = bp_progenitor();

		if ( ! empty( $bp_progenitor->user_feedback['message'] ) ) {
			$user_feedback = $bp_progenitor->user_feedback['message'];
		...
		} elseif ( ! empty( $bp_progenitor->template_message['message'] ) ) {
			/**
			 ...
			 */
			return apply_filters( 'bp_core_render_message_content', $bp_progenitor->template_message['message'], bp_progenitor_get_template_message_type() );
		}
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_template_notices() {
	$bp         = buddypress();
	$bp_progenitor = bp_progenitor();
	...
	if ( ! empty( $bp->template_message ) ) {
		...
		$bp_progenitor->template_message = $template_message;
		bp_get_template_part( 'common/notices/template-notices' );

		// Reset just after rendering it.
		$bp_progenitor->template_message = array();
 ...
 */
function bp_progenitor_user_feedback( $feedback_id = '' ) {
	if ( ! isset( $feedback_id ) ) {
		return;
	}

	$bp_progenitor = bp_progenitor();
	$feedback   = bp_progenitor_get_user_feedback( $feedback_id );
	...
	$bp_progenitor->user_feedback = $feedback;
	...
	bp_get_template_part(
		 ...
		 */
		apply_filters( 'bp_progenitor_user_feedback_template', 'common/notices/template-notices' )
	);
	...
	$bp_progenitor->user_feedback =array();
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_before_loop() {
 ...
 * @since 1.0.0
 */
function bp_progenitor_after_loop() {
	$component = bp_current_component();
 ...
 * @since 1.0.0
 */
function bp_progenitor_pagination( $position ) {
	$component = bp_current_component();
 ...
 */
function bp_progenitor_loop_classes() {
	echo esc_attr( bp_progenitor_get_loop_classes() );
}
	 ...
	 */
	function bp_progenitor_get_loop_classes() {
		$bp_progenitor = bp_progenitor();
		...
		if ( ! empty( $available_components[ $component ] ) && ( bp_is_directory() || bp_is_group() || bp_is_user() ) ) {
			...
			$layout_prefs      = bp_progenitor_get_temporary_setting(
				$customizer_option,
				bp_progenitor_get_appearance_settings( $customizer_option )
			);

			if ( $layout_prefs && (int) $layout_prefs > 1 ) {
				$grid_classes = bp_progenitor_customizer_grid_choices( 'classes' );
				...
				$bp_progenitor->{$component}->loop_layout = $layout_prefs;
			}
		}
		 ...
		 */
		$class_list = (array) apply_filters( 'bp_progenitor_get_loop_classes', $classes, $component );
 ...
 */
function bp_progenitor_loop_is_grid() {
	$bp_progenitor = bp_progenitor();
	$component  = sanitize_key( bp_current_component() );

	return ! empty( $bp_progenitor->{$component}->loop_layout ) && $bp_progenitor->{$component}->loop_layout > 1;
}
 ...
 */
function bp_progenitor_loop_get_grid_columns() {
	$bp_progenitor = bp_progenitor();
	$component  = sanitize_key( bp_current_component() );

	$columns = 1;

	if ( ! empty( $bp_progenitor->{$component}->loop_layout ) ) {
		$columns = (int) $bp_progenitor->{$component}->loop_layout;
	}
	 ...
	 */
	return (int) apply_filters( 'bp_progenitor_loop_get_grid_columns', $columns );
}
...
function bp_dir_is_vert_layout() {
		...
		return (bool) $bp_progenitor->{$component}->directory_vertical_layout;
}
 ...
 */
function bp_progenitor_avatar_args() {
	/**
	 * Filter arguments for full-size avatars.
	 *
	 * @param array $args
	 */
	return apply_filters( 'bp_progenitor_avatar_args', array(
		'type'   => 'full',
...
/*
 * This is the BP Progenitor Navigation Loop.
 *
 ...
 */
function bp_progenitor_has_nav( $args = array() ) {
	$bp_progenitor = bp_progenitor();
	...
	$nav                       = array();
	$bp_progenitor->displayed_nav = '';
	$bp_progenitor->object_nav    = $n['object'];

	if ( bp_is_directory() || 'directory' === $bp_progenitor->object_nav ) {
		$bp_progenitor->displayed_nav = 'directory';
		$nav                       = $bp_progenitor->directory_nav->get_primary();

	// So far it's only possible to build a Group nav when displaying it.
	} elseif ( bp_is_group() ) {
		$bp_progenitor->displayed_nav = 'groups';
		$parent_slug               = bp_get_current_group_slug();
		$group_nav                 = buddypress()->groups->nav;

		if ( 'group_manage' === $bp_progenitor->object_nav && bp_is_group_admin_page() ) {
			$parent_slug .= '_manage';
		 ...
		 */
			...
			bp_progenitor_set_nav_item_order( $group_nav, bp_progenitor_get_appearance_settings( 'group_nav_order' ), $parent_slug );
		}
	...
	// Build the nav for the displayed user
		...
		$bp_progenitor->displayed_nav = 'personal';
		...
		if ( 'secondary' === $n['type'] ) {
			...
			bp_progenitor_set_nav_item_order( $user_nav, bp_progenitor_get_appearance_settings( 'user_nav_order' ) );

			$nav = $user_nav->get_primary( $args );
		}

	} elseif ( ! empty( $bp_progenitor->object_nav ) ) {
		$bp_progenitor->displayed_nav = $bp_progenitor->object_nav;
		 ...
		 */
		$nav = apply_filters( 'bp_progenitor_get_nav', $nav, $n );
	}

	$bp_progenitor->sorted_nav = array_values( $nav );

	if ( 0 === count( $bp_progenitor->sorted_nav ) || ! $bp_progenitor->displayed_nav ) {
		unset( $bp_progenitor->sorted_nav, $bp_progenitor->displayed_nav, $bp_progenitor->object_nav );

		return false;
	}

	$bp_progenitor->current_nav_index = 0;
	return true;
}
 ...
 */
function bp_progenitor_nav_items() {
	$bp_progenitor = bp_progenitor();

	if ( isset( $bp_progenitor->sorted_nav[ $bp_progenitor->current_nav_index ] ) ) {
		return true;
	}

	$bp_progenitor->current_nav_index = 0;
	unset( $bp_progenitor->current_nav_item );
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_item() {
	$bp_progenitor = bp_progenitor();

	$bp_progenitor->current_nav_item   = $bp_progenitor->sorted_nav[ $bp_progenitor->current_nav_index ];
	$bp_progenitor->current_nav_index += 1;
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_id() {
	echo esc_attr( bp_progenitor_get_nav_id() );
}
	 ...
	 */
	function bp_progenitor_get_nav_id() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;

		if ( 'directory' === $bp_progenitor->displayed_nav ) {
			...
			$id = sprintf( '%1$s-%2$s-li', $nav_item->css_id, $bp_progenitor->displayed_nav );
		} else {
			$id = $nav_item->slug;
		}
		 ...
		 */
		return apply_filters( 'bp_progenitor_get_nav_id', $id, $nav_item, $bp_progenitor->displayed_nav );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_classes() {
	echo esc_attr( bp_progenitor_get_nav_classes() );
}
	 ...
	 */
	function bp_progenitor_get_nav_classes() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		$classes    = array();
		...
		if ( 'directory' === $bp_progenitor->displayed_nav && ! empty( $nav_item->li_class ) ) {
			...
			// Group Admin Tabs.
			} elseif ( 'group_manage' === $bp_progenitor->object_nav ) {
				$selected = bp_action_variable( 0 );
				$classes  = array( 'bp-' . $bp_progenitor->displayed_nav . '-admin-tab' );

			// If we are here, it's the member's subnav
			} elseif ( 'personal' === $bp_progenitor->displayed_nav ) {
				$classes  = array( 'bp-' . $bp_progenitor->displayed_nav . '-sub-tab' );
			}
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		$classes_list = apply_filters( 'bp_progenitor_get_classes', join( ' ', $classes ), $classes, $nav_item, $bp_progenitor->displayed_nav );
		if ( ! $classes_list ) {
			$classes_list = '';
		}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_scope() {
	echo bp_progenitor_get_nav_scope();  // Escaped by bp_get_form_field_attributes().
}
	 ...
	 */
	function bp_progenitor_get_nav_scope() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		$scope      = array();
		...
		if ( 'directory' === $bp_progenitor->displayed_nav ) {
		...
		} elseif ( 'personal' === $bp_progenitor->displayed_nav && ! empty( $nav_item->secondary ) ) {
			 ...
			 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
			 */
			$scope = apply_filters( 'bp_progenitor_set_nav_scope', $scope, $nav_item, $bp_progenitor->displayed_nav );
		}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_link() {
	echo esc_url( bp_progenitor_get_nav_link() );
}
	 ...
	 */
	function bp_progenitor_get_nav_link() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		...
		if ( 'personal' === $bp_progenitor->displayed_nav && ! empty( $nav_item->primary ) ) {
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'bp_progenitor_get_nav_link', $link, $nav_item, $bp_progenitor->displayed_nav );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_link_id() {
	echo esc_attr( bp_progenitor_get_nav_link_id() );
}
	 ...
	 */
	function bp_progenitor_get_nav_link_id() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		$link_id   = '';

		if ( ( 'groups' === $bp_progenitor->displayed_nav || 'personal' === $bp_progenitor->displayed_nav ) && ! empty( $nav_item->css_id ) ) {
			$link_id = $nav_item->css_id;

			if ( ! empty( $nav_item->primary ) && 'personal' === $bp_progenitor->displayed_nav ) {
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'bp_progenitor_get_nav_link_id', $link_id, $nav_item, $bp_progenitor->displayed_nav );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_link_title() {
	echo esc_attr( bp_progenitor_get_nav_link_title() );
}
	 ...
	 */
	function bp_progenitor_get_nav_link_title() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		$title      = '';

		if ( 'directory' === $bp_progenitor->displayed_nav && ! empty( $nav_item->title ) ) {
			$title = $nav_item->title;

		} elseif (
			( 'groups' === $bp_progenitor->displayed_nav || 'personal' === $bp_progenitor->displayed_nav )
			&&
			! empty( $nav_item->name )
		) {
			$title = $nav_item->name;
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'bp_progenitor_get_nav_link_title', $title, $nav_item, $bp_progenitor->displayed_nav );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_link_text() {
	echo esc_html( bp_progenitor_get_nav_link_text() );
}
	 ...
	 */
	function bp_progenitor_get_nav_link_text() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		$link_text  = '';

		if ( 'directory' === $bp_progenitor->displayed_nav && ! empty( $nav_item->text ) ) {
		 ...
		 * @param string $value     The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return apply_filters( 'bp_progenitor_get_nav_link_text', $link_text, $nav_item, $bp_progenitor->displayed_nav );
	}
 ...
 * @return bool
 */
function bp_progenitor_nav_has_count() {
	$bp_progenitor = bp_progenitor();
	$nav_item   = $bp_progenitor->current_nav_item;
	$count      = false;
	...
	} elseif ( 'groups' === $bp_progenitor->displayed_nav && 'members' === $nav_item->slug ) {
		$count = 0 !== (int) groups_get_current_group()->total_member_count;
	} elseif ( 'personal' === $bp_progenitor->displayed_nav && ! empty( $nav_item->primary ) ) {
	 ...
	 * @param string $value     The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
	 */
	return (bool) apply_filters( 'bp_progenitor_nav_has_count', false !== $count, $nav_item, $bp_progenitor->displayed_nav );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_nav_count() {
	echo esc_html( number_format_i18n( bp_progenitor_get_nav_count() ) );
}
	 ...
	 */
	function bp_progenitor_get_nav_count() {
		$bp_progenitor = bp_progenitor();
		$nav_item   = $bp_progenitor->current_nav_item;
		$count      = 0;
		 ...
		 * @param string $value    The current nav in use (eg: 'directory', 'groups', 'personal', etc..).
		 */
		return (int) apply_filters( 'bp_progenitor_get_nav_count', $count, $nav_item, $bp_progenitor->displayed_nav );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_directory_type_navs_class() {
	echo esc_attr( bp_progenitor_get_directory_type_navs_class() );
}
	 ...
	 * @return string
	 */
	function bp_progenitor_get_directory_type_navs_class() {
		$component  = sanitize_key( bp_current_component() );
		...
		$nav_style  = bp_progenitor_get_temporary_setting( $customizer_option, bp_progenitor_get_appearance_settings( $customizer_option ) );
		$tab_style = '';
		 ...
		 */
		$nav_wrapper_classes = (array) apply_filters( 'bp_progenitor_get_directory_type_navs_class', $nav_wrapper_classes );
 ...
 * @since 1.0.0
 */
function bp_progenitor_directory_list_class() {
	echo esc_attr( bp_progenitor_get_directory_list_class() );
}
	 ...
	 * @since 1.0.0
	 */
	function bp_progenitor_get_directory_list_class() {
 ...
 * @since 1.0.0
 */
function bp_progenitor_directory_nav_object() {
	$obj = bp_progenitor_get_directory_nav_object();
	 ...
	 * @return array
	 */
	function bp_progenitor_get_directory_nav_object() {
		$nav_item = bp_progenitor()->current_nav_item;
 ...
 */
function bp_progenitor_container_classes() {
	echo esc_attr( bp_progenitor_get_container_classes() );
}
	 ...
	 * @return string CSS classes
	 */
	function bp_progenitor_get_container_classes() {
		...
		if ( bp_is_user() ) {
			...
			$customizer_option = apply_filters( 'bp_progenitor_single_item_display_settings_id', '' );
		}
		...
		if ( 'register' === bp_current_component() && bp_is_active( 'xprofile' ) && bp_progenitor_base_account_has_xprofile()) {
		...
		$general_settings = bp_progenitor_get_temporary_setting( 'avatar_style', bp_progenitor_get_appearance_settings( 'avatar_style' ) );
		if ( $general_settings ) {
		...
		if ( $customizer_option ) {
			$layout_prefs  = bp_progenitor_get_temporary_setting( $customizer_option, bp_progenitor_get_appearance_settings( $customizer_option ) );
			...
			if ( $layout_prefs && bp_is_directory() ) {
				...
				$classes[] = 'bp-vertical-navs';
				$bp_progenitor->{$component}->directory_vertical_layout = $layout_prefs;
			}
		}

		$class = array_map( 'sanitize_html_class', $classes );

		return apply_filters( 'bp_progenitor_get_container_classes', join( ' ', $class ), $classes );
	}
 ...
 * @return string CSS classes
 */
function bp_progenitor_single_item_nav_classes() {
	echo esc_attr( bp_progenitor_get_single_item_nav_classes() );
}
	 ...
	 * @return string CSS classes
	 */
	function bp_progenitor_get_single_item_nav_classes() {
		...
		if ( bp_is_group() ) {
			$nav_tabs = bp_progenitor_get_temporary_setting( 'group_nav_tabs', bp_progenitor_get_appearance_settings( 'group_nav_tabs' ) );

		} elseif ( bp_is_user() ) {
			$nav_tabs = bp_progenitor_get_temporary_setting( 'user_nav_tabs', bp_progenitor_get_appearance_settings( 'user_nav_tabs' ) );
		}
		...
		// Set the global for a later use - this is moved from the `bp_progenitor_get_container_classes()
		// But was set as a check for this array class addition.
		$bp_progenitor->{$component}->single_primary_nav_layout = $layout_prefs;
		...
		return apply_filters( 'bp_progenitor_get_single_item_nav_classes', join( ' ', $class ), $classes );
	}
 ...
 * @return string CSS classes
 */
function bp_progenitor_single_item_subnav_classes() {
	echo esc_attr( bp_progenitor_get_single_item_subnav_classes() );
}
	 ...
	 * @return string CSS classes
	 */
	function bp_progenitor_get_single_item_subnav_classes() {
		...
		$nav_tabs = bp_progenitor_get_temporary_setting( $customizer_option, bp_progenitor_get_appearance_settings( $customizer_option ) );
		...
		return apply_filters( 'bp_progenitor_get_single_item_subnav_classes', join( ' ', $class ), $classes );
	}
 ...
 * @return string CSS classes
 */
function bp_progenitor_groups_create_steps_classes() {
	echo esc_attr( bp_progenitor_get_group_create_steps_classes() );
}
	 ...
	 * @return string CSS classes
	 */
	function bp_progenitor_get_group_create_steps_classes() {
		$classes  = array( 'bp-navs', 'group-create-links', 'no-ajax' );
		$nav_tabs = bp_progenitor_get_temporary_setting( 'groups_create_tabs', bp_progenitor_get_appearance_settings( 'groups_create_tabs' ) );
		...
		return apply_filters( 'bp_progenitor_get_group_create_steps_classes', join( ' ', $class ), $classes );
	}
 ...
 */
function bp_progenitor_get_search_primary_object( $object = '' ) {
	if ( bp_is_user() ) {
		$object = 'member';
	} elseif ( bp_is_group() ) {
		$object = 'group';
		...
		$object = apply_filters( 'bp_progenitor_get_search_primary_object', $object );
	}
 ...
 */
function bp_progenitor_get_search_objects( $objects = array() ) {
	$primary = bp_progenitor_get_search_primary_object();
	if ( ! $primary ) {
		return $objects;
	}
	...
	if ( 'member' === $primary || 'dir' === $primary ) {
		...
		$objects = apply_filters( 'bp_progenitor_get_search_objects', $objects );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_search_container_class() {
 ...
 */
function bp_progenitor_search_selector_id( $suffix = '', $sep = '-' ) {
	$id = join( $sep, array_merge( bp_progenitor_get_search_objects(), (array) $suffix ) );
	echo esc_attr( $id );
}
 ...
 */
function bp_progenitor_search_selector_name( $suffix = '', $sep = '_' ) {
	$objects = bp_progenitor_get_search_objects();
 ...
 * @since 1.0.0
 */
function bp_progenitor_search_form() {
 ...
 * @since 1.0.0
 */
function bp_progenitor_current_object() {
	/*
 ...
 * @since 1.0.0
 */
function bp_progenitor_filter_container_id() {
	echo esc_attr( bp_progenitor_get_filter_container_id() );
}
	 ...
	 * @param string
	 */
	function bp_progenitor_get_filter_container_id() {
		$component = bp_progenitor_current_object();
		...
		if ( isset( $ids[ $component['object'] ] ) ) {
			return apply_filters( 'bp_progenitor_get_filter_container_id', $ids[ $component['object'] ] );
		}
 ...
 * @since 1.0.0
 */
function bp_progenitor_filter_id() {
	echo esc_attr( bp_progenitor_get_filter_id() );
}
	 ...
	 * @param string
	 */
	function bp_progenitor_get_filter_id() {
		$component = bp_progenitor_current_object();
		...
		if ( isset( $ids[ $component['object'] ] ) ) {
			return apply_filters( 'bp_progenitor_get_filter_id', $ids[ $component['object'] ] );
		}
 ...
 * @since 1.0.0
 */
function bp_progenitor_filter_label() {
	echo esc_html( bp_progenitor_get_filter_label() );
}
	 ...
	 * @param string
	 */
	function bp_progenitor_get_filter_label() {
		$component = bp_progenitor_current_object();
		...
		return apply_filters( 'bp_progenitor_get_filter_label', $label );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_filter_component() {
	$component = bp_progenitor_current_object();
 ...
 * @since 1.0.0
 */
function bp_progenitor_filter_options() {
	echo bp_progenitor_get_filter_options();  // Escaped in inner functions.
}
	 ...
	 * @return string
	 */
	function bp_progenitor_get_filter_options() {
		$output = '';

		if ( 'notifications' === bp_current_component() ) {
			$output = bp_progenitor_get_notifications_filters();

		} else {
			$filters = bp_progenitor_get_component_filters();
 ...
 * @return string HTML.
 */
function bp_progenitor_get_customizer_link( $args = array() ) {
	$r = bp_parse_args( $args, array(
		...
		'autofocus'  => '',
		'text'       => '',
	), 'progenitor_get_customizer_link' );
 ...
 */
function bp_progenitor_signup_hook( $when = '', $prefix = '' ) {
	$hook = array( 'bp' );
 ...
 * @param string $prefix Use it to add terms before the hook name
 */
function bp_progenitor_activation_hook( $when = '', $suffix = '' ) {
	$hook = array( 'bp' );
 ...
 */
function bp_progenitor_submit_button( $action ) {
	$submit_data = bp_progenitor_get_submit_button( $action );
