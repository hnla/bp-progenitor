 ...
 * @since 1.0.0
 */
function bp_progenitor_get_blogs_directory_nav_items() {
	$nav_items = array();
	...
	$extra_nav_items = bp_progenitor_parse_hooked_dir_nav( 'bp_blogs_directory_blog_types', 'blogs', 20 );
	 ...
	 */
	return apply_filters( 'bp_progenitor_get_blogs_directory_nav_items', $nav_items );
}
 ...
 */
function bp_progenitor_get_blogs_filters( $context = '' ) {
	if ( empty( $context ) ) {
		return array();
	}
	 ...
	 */
	$filters = apply_filters( 'bp_progenitor_get_blogs_filters', array(
 ...
 */
function bp_progenitor_blogs_catch_button_args( $button = array() ) {
	// Globalize the arguments so that we can use it  in bp_progenitor_get_blogs_buttons().
	bp_progenitor()->blogs->button_args = $button;
 ...
 */
function bp_progenitor_blogs_customizer_settings( $settings = array() ) {
	return array_merge( $settings, array(
		'bp_progenitor_appearance[blogs_layout]' => array(
			'index'             => 'blogs_layout',
 ...
 */
function bp_progenitor_get_blog_signup_inline_script() {
	return '
		( function( $ ) {
 ...
 * @since 1.2.0
 */
function bp_progenitor_blog_loop_item_has_lastest_post( $classes) {
...
add_filter( 'bp_get_blog_class', 'bp_progenitor_blog_loop_item_has_lastest_post' );
