 ...
 * @since 1.0.0
 */
function bp_progenitor_before_blogs_directory_content() {
	/**
 ...
 * @since 1.0.0
 */
function bp_progenitor_after_blogs_directory_content() {
	/**
 ...
 */
function bp_progenitor_blogs_create_hook( $when = '', $suffix = '' ) {
	$hook = array( 'bp' );
 ...
 * @since 1.0.0
 */
function bp_progenitor_blogs_loop_item() {
	/**
 ...
 */
function bp_progenitor_blogs_loop_buttons( $args = array() ) {
	...
	$output = join( ' ', bp_progenitor_get_blogs_buttons( $args ) );
	 ...
	 * @return array
	 */
	function bp_progenitor_get_blogs_buttons( $args ) {
		 ...
		 */
		add_filter( 'bp_get_blogs_visit_blog_button', 'bp_progenitor_blogs_catch_button_args', 100, 1 );

		bp_get_blogs_visit_blog_button();

		remove_filter( 'bp_get_blogs_visit_blog_button', 'bp_progenitor_blogs_catch_button_args', 100, 1 );

		if ( ! empty( bp_progenitor()->blogs->button_args ) ) {
			$button_args = bp_progenitor()->blogs->button_args ;
			...
			unset( bp_progenitor()->blogs->button_args );
		}
		 ...
		 */
		$buttons_group = apply_filters( 'bp_progenitor_get_blogs_buttons', $buttons, $blog, $type );
		...
		if ( ! isset( bp_progenitor()->blogs->group_buttons ) || ! is_a( bp_progenitor()->blogs->group_buttons, 'BP_Buttons_Group' ) ) {
			$sort = true;
			bp_progenitor()->blogs->group_buttons = new BP_Buttons_Group( $buttons_group );

		// It's not the first entry, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			bp_progenitor()->blogs->group_buttons->update( $buttons_group );
		}

		$return = bp_progenitor()->blogs->group_buttons->get( $sort );
		 ...
		 * @param string $type    Whether we're displaying a blogs loop or a the blogs single item (in the future!).
		 */
		do_action_ref_array( 'bp_progenitor_return_blogs_buttons', array( &$return, $blog, $type ) );
 ...
 */
function bp_progenitor_blog_has_latest_post() {
