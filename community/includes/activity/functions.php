 ...
 */
function bp_progenitor_activity_register_scripts( $scripts = array() ) {
	if ( ! isset( $scripts['bp-progenitor'] ) ) {
		return $scripts;
	}

	return array_merge( $scripts, array(
		'bp-progenitor-activity' => array(
			'file'         => 'js/buddypress-activity%s.js',
			'dependencies' => array( 'bp-progenitor' ),
			'footer'       => true,
		),
		'bp-progenitor-activity-post-form' => array(
			'file'         => 'js/buddypress-activity-post-form%s.js',
			'dependencies' => array( 'bp-progenitor', 'bp-progenitor-activity', 'json2', 'wp-backbone' ),
			'footer'       => true,
		),
	) );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_enqueue_scripts() {
	if ( ! bp_is_activity_component() && ! bp_is_group_activity() ) {
		return;
	}

	wp_enqueue_script( 'bp-progenitor-activity' );
}
 ...
 */
function bp_progenitor_activity_localize_scripts( $params = array() ) {
	if ( ! bp_is_activity_component() && ! bp_is_group_activity() ) {
		return $params;
	}
	 ...
	 * );
	 */
	$activity_buttons = apply_filters( 'bp_progenitor_activity_buttons', array() );
	...
	if ( ! bp_is_single_item() && ! bp_is_user() ) {
		...
		$activity_params['objects'] = apply_filters( 'bp_progenitor_activity_objects', $activity_objects );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_get_activity_directory_nav_items() {
	$nav_items = array();
	...
	foreach ( $deprecated_hooks as $deprectated_hook ) {
		...
		$extra_nav_items = bp_progenitor_parse_hooked_dir_nav( $hook, $component, $position );
	 ...
	 */
	return apply_filters( 'bp_progenitor_get_activity_directory_nav_items', $nav_items );
}
 ...
 * @return array
 */
function bp_progenitor_get_activity_filters_array( $output = '', $filters = array(), $context = '' ) {
 ...
 */
function bp_progenitor_get_activity_filters() {
	add_filter( 'bp_get_activity_show_filters', 'bp_progenitor_get_activity_filters_array', 10, 3 );

	$filters_data = bp_get_activity_show_filters();

	remove_filter( 'bp_get_activity_show_filters', 'bp_progenitor_get_activity_filters_array', 10, 3 );
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_secondary_avatars( $action, $activity ) {
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_scope_newest_class( $classes = '' ) {
	if ( ! is_user_logged_in() ) {
		return $classes;
	}
	 ...
	 * See bp_progenitor_ajax_querystring().
	 */
	$scope = '';
	...
	if ( $scope && bp_is_activity_directory() ) {
		...
		$my_classes = (array) apply_filters( 'bp_progenitor_activity_scope_newest_class', $my_classes, $scope );
...
function bp_progenitor_activity_time_since( $time_since, $activity = null ) {
	if ( ! isset ( $activity->date_recorded ) ) {
		return $time_since;
	}

	return apply_filters( 'bp_progenitor_activity_time_since', sprintf(
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_allowed_tags( $activity_allowedtags = array() ) {
 ...
 */
function bp_progenitor_activity_widget_query() {
	$args       = array();
	$bp_progenitor = bp_progenitor();

	if ( isset( $bp_progenitor->activity->widget_args ) ) {
		$args = $bp_progenitor->activity->widget_args;
	}
	 ...
	 */
	return apply_filters( 'bp_progenitor_activity_widget_query', $args );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_notification_filters() {
	$notifications = array(
		array(
			'id'       => 'new_at_mention',
