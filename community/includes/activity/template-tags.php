 ...
 * @since 1.0.0
 */
function bp_progenitor_before_activity_directory_content() {
	/**
 ...
 * @since 1.0.0
 */
function bp_progenitor_after_activity_directory_content() {
	/**
 ...
 * @since 1.0.0
 */
function bp_progenitor_before_activity_post_form() {
	if ( bp_progenitor_current_user_can( 'publish_activity' ) ) {
		wp_enqueue_script( 'bp-progenitor-activity-post-form' );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_after_activity_post_form() {
	if ( bp_progenitor_current_user_can( 'publish_activity' ) ) {
 ...
 * @return string HTML.
 */
function bp_progenitor_activity_member_post_form() {
 ...
 */
function bp_progenitor_activity_hook( $when = '', $suffix = '' ) {
	$hook = array( 'bp' );
 ...
 */
function bp_progenitor_activity_has_content() {
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_content() {
 ...
 */
function bp_progenitor_activity_entry_buttons( $args = array() ) {
	$output = join( ' ', bp_progenitor_get_activity_entry_buttons( $args ) );
	 ...
	 * @since 1.0.0
	 */
	function bp_progenitor_get_activity_entry_buttons( $args ) {
		$buttons = array();
		 ...
		 */
		$buttons_group = apply_filters( 'bp_progenitor_get_activity_entry_buttons', $buttons, $activity_id );
		...
		if ( ! isset( bp_progenitor()->activity->entry_buttons ) || ! is_a( bp_progenitor()->activity->entry_buttons, 'BP_Buttons_Group' ) ) {
			$sort = true;
			bp_progenitor()->activity->entry_buttons = new BP_Buttons_Group( $buttons_group );

		// It's not the first entry, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			bp_progenitor()->activity->entry_buttons->update( $buttons_group );
		}

		$return = bp_progenitor()->activity->entry_buttons->get( $sort );
		 ...
		 * @param int   $activity_id The current activity ID.
		 */
		do_action_ref_array( 'bp_progenitor_return_activity_entry_buttons', array( &$return, $activity_id ) );
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_comments() {
	...
	bp_progenitor_activity_recurse_comments( $activities_template->activity );
}
 ...
 */
function bp_progenitor_activity_recurse_comments( $comment ) {
	global $activities_template;
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_comment_action() {
	echo bp_progenitor_get_activity_comment_action();
}
	 ...
	 * @since 1.0.0
	 */
	function bp_progenitor_get_activity_comment_action() {
		 ...
		 */
		return apply_filters( 'bp_progenitor_get_activity_comment_action',
 ...
 * @since 1.0.0
 */
function bp_progenitor_activity_comment_form() {
 ...
 */
function bp_progenitor_activity_comment_buttons( $args = array() ) {
	$output = join( ' ', bp_progenitor_get_activity_comment_buttons( $args ) );
	...
	function bp_progenitor_get_activity_comment_buttons( $args ) {
		 ...
		 * @param int   $activity_id         The current activity ID.
		 */
		$buttons_group = apply_filters( 'bp_progenitor_get_activity_comment_buttons', $buttons, $activity_comment_id, $activity_id );
		...
		if ( ! isset( bp_progenitor()->activity->comment_buttons ) || ! is_a( bp_progenitor()->activity->comment_buttons, 'BP_Buttons_Group' ) ) {
			$sort = true;
			bp_progenitor()->activity->comment_buttons = new BP_Buttons_Group( $buttons_group );

		// It's not the first comment, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			bp_progenitor()->activity->comment_buttons->update( $buttons_group );
		}

		$return = bp_progenitor()->activity->comment_buttons->get( $sort );
		 ...
		 * @param int   $activity_id         The current activity ID.
		 */
		do_action_ref_array( 'bp_progenitor_return_activity_comment_buttons', array( &$return, $activity_comment_id, $activity_id ) );
