 ...
 * @since 1.0.0
 */
function bp_progenitor_members_enqueue_scripts() {
	// Neutralize Ajax when using BuddyPress Groups & member widgets on default front page
	if ( ! bp_is_user_front() || ! bp_progenitor_get_appearance_settings( 'user_front_page' ) ) {
		return;
	}
 ...
 */
function bp_progenitor_get_members_directory_nav_items() {
	$nav_items = array();
	...
	$extra_nav_items = bp_progenitor_parse_hooked_dir_nav( 'bp_members_directory_member_types', 'members', 20 );
	 ...
	 */
	return apply_filters( 'bp_progenitor_get_members_directory_nav_items', $nav_items );
}
 ...
 */
function bp_progenitor_get_members_filters( $context = '' ) {
	if ( 'group' !== $context ) {
		$filters = array(
	 ...
	 */
	$filters = apply_filters( 'bp_progenitor_get_members_filters', $filters, $context );
 ...
 */
function bp_progenitor_members_catch_button_args( $button = array() ) {
	/*
	 * Globalize the arguments so that we can use it
	 * in bp_progenitor_get_member_header_buttons().
	 */
	bp_progenitor()->members->button_args = $button;
 ...
 */
function bp_progenitor_get_hooked_member_meta() {
	ob_start();
 ...
 */
function bp_progenitor_member_reset_front_template( $templates = array() ) {
	$use_default_front = bp_progenitor_get_appearance_settings( 'user_front_page' );

	// Setting the front template happens too early, so we need this!
	if ( is_customize_preview() ) {
		$use_default_front = bp_progenitor_get_temporary_setting( 'user_front_page', $use_default_front );
	}
	...
	return apply_filters( '_bp_progenitor_member_reset_front_template', $templates );
}
 ...
 */
function bp_progenitor_member_restrict_user_front_templates( $templates = array() ) {
 ...
 */
function bp_progenitor_member_locate_template_part( $template = '' ) {
	...
	if ( ! isset( $bp_progenitor->members->displayed_user_hierarchy ) ) {
		$bp_progenitor->members->displayed_user_hierarchy = array(
		...
		if ( bp_get_member_types() ) {
			...
			$bp_progenitor->members->displayed_user_hierarchy[] = 'members/single/%s-member-type-' . sanitize_file_name( $displayed_user_member_type )   . '.php';
		}

		// And the regular one
		$bp_progenitor->members->displayed_user_hierarchy[] = 'members/single/%s.php';
	}

	$templates = array();

	// Loop in the hierarchy to fill it for the requested template part
	foreach ( $bp_progenitor->members->displayed_user_hierarchy as $part ) {
		$templates[] = sprintf( $part, $template );
	}

	return bp_locate_template( apply_filters( 'bp_progenitor_member_locate_template_part', $templates ), false, true );
}
 ...
 */
function bp_progenitor_member_get_template_part( $template = '' ) {
	$located = bp_progenitor_member_locate_template_part( $template );
 ...
 */
function bp_progenitor_members_wp_bio_info() {
	$user_settings = bp_progenitor_get_appearance_settings();
 ...
 */
function bp_progenitor_member_is_home_widgets() {
	return true === bp_progenitor()->members->is_user_home_sidebar;
}
 ...
 */
function bp_progenitor_member_activity_widget_overrides( $args = array() ) {
 ...
 */
function bp_progenitor_member_groups_widget_overrides( $args = array() ) {
	return array_merge( $args, array(
		'user_id' => bp_displayed_user_id(),
	) );
}
 ...
 */
function bp_progenitor_member_members_widget_overrides( $args = array() ) {
 ...
 * @since 1.0.0
 */
function bp_progenitor_members_add_home_widget_filters() {
	add_filter( 'bp_progenitor_activity_widget_query', 'bp_progenitor_member_activity_widget_overrides', 10, 1 );
	add_filter( 'bp_before_has_groups_parse_args',  'bp_progenitor_member_groups_widget_overrides',   10, 1 );
	add_filter( 'bp_before_has_members_parse_args', 'bp_progenitor_member_members_widget_overrides',  10, 1 );

	do_action( 'bp_progenitor_members_add_home_widget_filters' );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_members_remove_home_widget_filters() {
	remove_filter( 'bp_progenitor_activity_widget_query', 'bp_progenitor_member_activity_widget_overrides', 10, 1 );
	remove_filter( 'bp_before_has_groups_parse_args',  'bp_progenitor_member_groups_widget_overrides',   10, 1 );
	remove_filter( 'bp_before_has_members_parse_args', 'bp_progenitor_member_members_widget_overrides',  10, 1 );

	do_action( 'bp_progenitor_members_remove_home_widget_filters' );
}
 ...
 */
function bp_progenitor_get_wp_profile_fields( $user = null ) {
	// Leave a chance to plugins to avoid showing the contact methods they're adding on front end.
	$contact_methods = (array) apply_filters( 'bp_progenitor_get_wp_profile_field', wp_get_user_contact_methods( $user ), $user );
 ...
 */
function bp_progenitor_member_customizer_nav() {
	add_filter( '_bp_progenitor_member_reset_front_template', 'bp_progenitor_member_restrict_user_front_templates', 10, 1 );
	...
	remove_filter( '_bp_progenitor_member_reset_front_template', 'bp_progenitor_member_restrict_user_front_templates', 10, 1 );

	$nav = buddypress()->members->nav;

	// Eventually reset the order.
	bp_progenitor_set_nav_item_order( $nav, bp_progenitor_get_appearance_settings( 'user_nav_order' ) );
