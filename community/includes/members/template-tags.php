 ...
 * @since 1.0.0
 */
function bp_progenitor_before_members_directory_content() {
	/**
 ...
 * @since 1.0.0
 */
function bp_progenitor_after_members_directory_content() {
	/**
 ...
 */
function bp_progenitor_member_hook( $when = '', $suffix = '' ) {
	$hook = array( 'bp' );
 ...
 * @since 1.0.0
 */
function bp_progenitor_member_email_notice_settings() {
	/**
 ...
 * @since 1.0.0
 *
 * @param array $args See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_member_header_buttons( $args = array() ) {
	...
	$output = join( ' ', bp_progenitor_get_members_buttons( $args ) );
	 ...
	 */
	if ( ! empty( $bp_progenitor->members->member_buttons ) ) {
		unset( $bp_progenitor->members->member_buttons );
	}
 ...
 * @param array $args See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_members_loop_buttons( $args = array() ) {
	 ...
	 * @return array
	 */
	function bp_progenitor_get_members_buttons( $args ) {
		$buttons = array();
		...
		if ( bp_is_active( 'friends' ) ) {
			...
			// It's any other members screen
				...
				if ( ! empty( bp_progenitor()->members->button_args ) ) {
					$button_args = bp_progenitor()->members->button_args;
					...
					unset( bp_progenitor()->members->button_args );
				}
			}
		}
		...
		if ( 'profile' === $type ) {
			if ( bp_is_active( 'activity' ) && bp_activity_do_mentions() ) {
				 ...
				 * See https://buddypress.trac.wordpress.org/ticket/7126
				 */
				add_filter( 'bp_get_send_public_message_button', 'bp_progenitor_members_catch_button_args', 100, 1 );

				bp_get_send_public_message_button();

				remove_filter( 'bp_get_send_public_message_button', 'bp_progenitor_members_catch_button_args', 100, 1 );
			...
			if ( bp_is_active( 'messages' ) ) {
				 ...
				 * @see https://buddypress.trac.wordpress.org/ticket/7126
				 */
				add_filter( 'bp_get_send_message_button_args', 'bp_progenitor_members_catch_button_args', 100, 1 );

				bp_get_send_message_button();

				remove_filter( 'bp_get_send_message_button_args', 'bp_progenitor_members_catch_button_args', 100, 1 );
		 ...
		 */
		$buttons_group = apply_filters( 'bp_progenitor_get_members_buttons', $buttons, $user_id, $type );
		if ( ! $buttons_group ) {
			return $buttons;
		}

		// It's the first entry of the loop, so build the Group and sort it
		if ( ! isset( bp_progenitor()->members->member_buttons ) || ! is_a( bp_progenitor()->members->member_buttons, 'BP_Buttons_Group' ) ) {
			$sort = true;
			bp_progenitor()->members->member_buttons = new BP_Buttons_Group( $buttons_group );

		// It's not the first entry, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			bp_progenitor()->members->member_buttons->update( $buttons_group );
		}

		$return = bp_progenitor()->members->member_buttons->get( $sort );
		 ...
		 * @param string $type    Whether we're displaying a members loop or a user's page
		 */
		do_action_ref_array( 'bp_progenitor_return_members_buttons', array( &$return, $user_id, $type ) );
 ...
 */
function bp_progenitor_member_has_meta() {
	return (bool) bp_progenitor_get_member_meta();
}
 ...
 */
function bp_progenitor_member_meta() {
	echo join( "\n", bp_progenitor_get_member_meta() );
}
	 ...
	 */
	function bp_progenitor_get_member_meta() {
		$meta    = array();
		$is_loop = false;
		...
		if ( empty( $member->template_meta ) ) {
			...
			$extra_meta = bp_progenitor_get_hooked_member_meta();
			 ...
			 */
			$member->template_meta = apply_filters( 'bp_progenitor_get_member_meta', $meta, $member, $is_loop );
		}
 ...
 * @since 1.0.0
 */
function bp_progenitor_member_template_part() {
	/**
	...
	if ( bp_is_user_front() ) {
		...
		bp_progenitor_member_get_template_part( $template );
	}
 ...
 * @return string HTML Output
 */
function bp_progenitor_member_header_template_part() {
	...
	bp_progenitor_template_notices();
}
 ...
 * @return string HTML Output
 */
function bp_progenitor_members_get_customizer_option_link() {
	return bp_progenitor_get_customizer_link( array(
		'object'    => 'user',
		'autofocus' => 'bp_progenitor_user_front_page',
 ...
 * @return string HTML Output
 */
function bp_progenitor_members_get_customizer_widgets_link() {
	return bp_progenitor_get_customizer_link( array(
		'object'    => 'user',
 ...
 * @return string HTML output.
 */
function bp_progenitor_member_description( $user_id = 0 ) {
	if ( ! $user_id ) {
 ...
 * @return string HTML Output
 */
function bp_progenitor_member_description_edit_link() {
	echo bp_progenitor_member_get_description_edit_link();
}
	 ...
	 * @return string HTML Output
	 */
	function bp_progenitor_member_get_description_edit_link() {
 ...
 * @since 1.0.0
 */
function bp_progenitor_wp_profile_hooks( $type = 'before' ) {
	if ( 'before' === $type ) {
		/**
 ...
 */
function bp_progenitor_has_wp_profile_fields() {
	$user_id = bp_displayed_user_id();
	if ( ! $user_id ) {
		return false;
	}
	...
	$fields              = bp_progenitor_get_wp_profile_fields( $user );
	...
	$bp_progenitor = bp_progenitor();
	$bp_progenitor->members->wp_profile       = $user_profile_fields;
	$bp_progenitor->members->wp_profile_index = 0;
 ...
 */
function bp_progenitor_wp_profile_fields() {
	$bp_progenitor = bp_progenitor();

	if ( isset( $bp_progenitor->members->wp_profile[ $bp_progenitor->members->wp_profile_index ] ) ) {
		return true;
	}

	$bp_progenitor->members->wp_profile_index = 0;
	unset( $bp_progenitor->members->wp_profile_current );
 ...
 * @since 1.0.0
 */
function bp_progenitor_wp_profile_field() {
	$bp_progenitor = bp_progenitor();

	$bp_progenitor->members->wp_profile_current = $bp_progenitor->members->wp_profile[ $bp_progenitor->members->wp_profile_index ];
	$bp_progenitor->members->wp_profile_index += 1;
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_wp_profile_field_id() {
	echo esc_attr( bp_progenitor_get_wp_profile_field_id() );
}
	/**
	 ...
	 */
	function bp_progenitor_get_wp_profile_field_id() {
		$field = bp_progenitor()->members->wp_profile_current;
		return apply_filters( 'bp_progenitor_get_wp_profile_field_id', $field->id );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_wp_profile_field_label() {
	echo esc_html( bp_progenitor_get_wp_profile_field_label() );
}
	 ...
	 */
	function bp_progenitor_get_wp_profile_field_label() {
		$field = bp_progenitor()->members->wp_profile_current;
		return apply_filters( 'bp_progenitor_get_wp_profile_field_label', $field->label );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_wp_profile_field_data() {
	$data = bp_progenitor_get_wp_profile_field_data();
	...
	return wp_kses(
		 ...
		 */
		apply_filters( 'bp_progenitor_get_wp_profile_field_data', $data ),
	 ...
	 */
	function bp_progenitor_get_wp_profile_field_data() {
		$field = bp_progenitor()->members->wp_profile_current;
		return $field->data;
	}
