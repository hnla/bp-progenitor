...
class BP_Buttons_Group {
	...
	public function __construct( $args = array() ) {
		...
		foreach ( $args as $arg ) {
			...
			/*
			 * If, in bp_progenitor_get_*_buttons(), we pass through a false value for 'parent_element'
 ...
 * @uses   WP_Widget
 */
class BP_Progenitor_Object_Nav_Widget extends WP_Widget {
	/**
	 * Constructor
	 *
	 * @since 1.0
	...
	public function __construct() {
		...
		parent::__construct(
			'bp_progenitor_sidebar_object_nav_widget',
	...
	public static function register_widget() {
		register_widget( 'BP_Progenitor_Object_Nav_Widget' );
	}
	...
	public function widget( $args, $instance ) {
		...
		$item_nav_args = wp_parse_args( $instance, apply_filters( 'bp_progenitor_object_nav_widget_args', array(
			'bp_progenitor_widget_title' => true,
		) ) );

		$title = '';

		if ( ! empty( $item_nav_args[ 'bp_progenitor_widget_title' ] ) ) {
			$title = '';
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['bp_progenitor_widget_title'] = (bool) $new_instance['bp_progenitor_widget_title'];
	...
	public function form( $instance ) {
		...
		$bp_progenitor_widget_title = (bool) $instance['bp_progenitor_widget_title'];
		?>

		<p>
			<input class="checkbox" type="checkbox" <?php checked( $bp_progenitor_widget_title, true ) ?> id="<?php echo $this->get_field_id( 'bp_progenitor_widget_title' ); ?>" name="<?php echo $this->get_field_name( 'bp_progenitor_widget_title' ); ?>" />
			<label for="<?php echo $this->get_field_id( 'bp_progenitor_widget_title' ); ?>"><?php esc_html_e( 'Include navigation title', 'buddypress' ); ?></label>
		</p>
