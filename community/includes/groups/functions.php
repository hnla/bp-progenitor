 ...
 * @since 3.0
 */
function bp_progenitor_message_markup_wrapper( $message, $type ) {
 ...
 */
function bp_progenitor_groups_register_scripts( $scripts = array() ) {
	if ( ! isset( $scripts['bp-progenitor'] ) ) {
		return $scripts;
	}

	return array_merge( $scripts, array(
		'bp-progenitor-group-invites' => array(
			'file'         => 'js/buddypress-group-invites%s.js',
			'dependencies' => array( 'bp-progenitor', 'json2', 'wp-backbone' ),
			'footer'       => true,
		),
	) );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_enqueue_scripts() {
	// Neutralize Ajax when using BuddyPress Groups & member widgets on default front page
	if ( bp_is_group_home() && bp_progenitor_get_appearance_settings( 'group_front_page' ) ) {
		wp_add_inline_style( 'bp-progenitor', '
	...
	wp_enqueue_script( 'bp-progenitor-group-invites' );
}
 ...
 * @return bool
 */
function bp_progenitor_groups_disallow_all_members_invites( $default = false ) {
	/**
	 ...
	 */
	return apply_filters( 'bp_progenitor_groups_disallow_all_members_invites', $default );
}
 ...
 */
function bp_progenitor_groups_localize_scripts( $params = array() ) {
	if ( ! bp_is_group_invites() && ! ( bp_is_group_create() && bp_is_group_creation_step( 'group-invites' ) ) ) {
		return $params;
	}
	...
	if ( bp_is_active( 'friends' ) ) {
		...
		if ( true === bp_progenitor_groups_disallow_all_members_invites() ) {
	...
	$params['group_invites'] = array(
		...
		'loading'            => bp_progenitor_message_markup_wrapper( __( 'Loading members, please wait.', 'buddypress' ), 'loading' ),
		'invites_form'       => bp_progenitor_message_markup_wrapper( __( 'Use the "Send" button to send your invite, or the "Cancel" button to abort.', 'buddypress' ), 'info' ),
		'invites_form_reset' => bp_progenitor_message_markup_wrapper( __( 'Invites cleared, please use one of the available tabs to select members to invite.', 'buddypress' ), 'success' ),
		'invites_sending'    => bp_progenitor_message_markup_wrapper( __( 'Sending the invites, please wait.', 'buddypress' ), 'loading' ),
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_get_inviter_ids( $user_id, $group_id ) {
	if ( empty( $user_id ) || empty( $group_id ) ) {
		return false;
	}

	return BP_progenitor_Group_Invite_Query::get_inviter_ids( $user_id, $group_id );
}

/**
 * @since 1.0.0
 */
function bp_progenitor_prepare_group_potential_invites_for_js( $user ) {
	$bp = buddypress();
	...
	if ( ! empty( $bp->groups->invites_scope ) && 'invited' === $bp->groups->invites_scope ) {
		...
		$inviter_ids = bp_progenitor_groups_get_inviter_ids( $user->ID, bp_get_current_group_id() );
	...
	return apply_filters( 'bp_progenitor_prepare_group_potential_invites_for_js', $response, $user );
}

/**
 * @since 1.0.0
 */
function bp_progenitor_get_group_potential_invites( $args = array() ) {
	 ...
	 */
	if ( ! $r['user_id'] && bp_is_active( 'friends' ) && bp_is_active( 'settings' ) && ! bp_progenitor_groups_disallow_all_members_invites() ) {
		$r['meta_query'] = array(
			array(
				'key'     => '_bp_progenitor_restrict_invites_to_friends',
				'compare' => 'NOT EXISTS',
			),
		);
	}

	$query = new BP_progenitor_Group_Invite_Query( $r );
 ...
 */
function bp_progenitor_group_invites_create_steps( $steps = array() ) {
 ...
 * @since 1.0.0
 */
function bp_progenitor_group_setup_nav() {
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_invites_custom_message( $message = '' ) {
	if ( empty( $message ) ) {
		return $message;
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_prepare_group_for_js( $item ) {
	if ( empty( $item->id ) ) {
		return array();
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_invites_restriction_nav() {
	...
	bp_core_new_subnav_item( array(
		...
		'parent_slug'     => $slug,
		'screen_function' => 'bp_progenitor_groups_screen_invites_restriction',
		'item_css_id'     => 'invites',
		'position'        => 70,
 ...
 */
function bp_progenitor_groups_invites_restriction_admin_nav( $wp_admin_nav ) {
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_screen_invites_restriction() {
	...
	if ( isset( $_POST['member-group-invites-submit'] ) ) {
		// Nonce check.
		check_admin_referer( 'bp_progenitor_group_invites_settings' );

		if ( bp_is_my_profile() || bp_current_user_can( 'bp_moderate' ) ) {
			if ( empty( $_POST['account-group-invites-preferences'] ) ) {
				bp_delete_user_meta( bp_displayed_user_id(), '_bp_progenitor_restrict_invites_to_friends' );
			} else {
				bp_update_user_meta( bp_displayed_user_id(), '_bp_progenitor_restrict_invites_to_friends', (int) $_POST['account-group-invites-preferences'] );
			}
	 ...
	 */
	bp_core_load_template( apply_filters( 'bp_progenitor_groups_screen_invites_restriction', 'members/single/settings/group-invites' ) );
}

/**
 * @since 1.0.0
 */
function bp_progenitor_get_groups_directory_nav_items() {
	$nav_items = array();
	...
	$extra_nav_items = bp_progenitor_parse_hooked_dir_nav( 'bp_groups_directory_group_filter', 'groups', 20 );
	 ...
	 */
	return apply_filters( 'bp_progenitor_get_groups_directory_nav_items', $nav_items );
}
 ...
 */
function bp_progenitor_get_groups_filters( $context = '' ) {
	if ( empty( $context ) ) {
		return array();
	}
	 ...
	 */
	$filters = apply_filters( 'bp_progenitor_get_groups_filters', array(
	...
	if ( $action ) {
		return bp_progenitor_parse_hooked_options( $action, $filters );
	}
 ...
 */
function bp_progenitor_groups_catch_button_args( $button = array() ) {
	/**
	 * Globalize the arguments so that we can use it
	 * in bp_progenitor_get_groups_buttons().
	 */
	bp_progenitor()->groups->button_args = $button;
 ...
 */
function bp_progenitor_get_hooked_group_meta() {
	ob_start();
 ...
 */
function bp_progenitor_groups_do_group_boxes() {
	$group_settings = bp_progenitor_get_appearance_settings();
 ...
 * @return bool True to display. False otherwise.
 */
function bp_progenitor_groups_front_page_description() {
	$group_settings = bp_progenitor_get_appearance_settings();
 ...
 */
function bp_progenitor_groups_customizer_sections( $sections = array() ) {
	return array_merge( $sections, array(
		'bp_progenitor_group_front_page' => array(
			'title'       => __( 'Group\'s front page', 'buddypress' ),
			'panel'       => 'bp_progenitor_panel',
			'priority'    => 20,
			'description' => __( 'Set your preferences for the groups default front page.', 'buddypress' ),
		),
		'bp_progenitor_group_primary_nav' => array(
 ...
 */
function bp_progenitor_groups_customizer_settings( $settings = array() ) {
	return array_merge( $settings, array(
		'bp_progenitor_appearance[group_front_page]' => array(
		...
		'bp_progenitor_appearance[group_front_boxes]' => array(
		...
		'bp_progenitor_appearance[group_front_description]' => array(
		...
		'bp_progenitor_appearance[group_nav_display]' => array(
		...
		'bp_progenitor_appearance[group_nav_tabs]' => array(
		...
		'bp_progenitor_appearance[group_subnav_tabs]' => array(
		...
		'bp_progenitor_appearance[groups_create_tabs]' => array(
		...
		'bp_progenitor_appearance[group_nav_order]' => array(
			'index'             => 'group_nav_order',
			'capability'        => 'bp_moderate',
			'sanitize_callback' => 'bp_progenitor_sanitize_nav_order',
			'transport'         => 'refresh',
			'type'              => 'option',
		),
		'bp_progenitor_appearance[groups_layout]' => array(
			'index'             => 'groups_layout',
			'capability'        => 'bp_moderate',
			'sanitize_callback' => 'absint',
			'transport'         => 'refresh',
 ...
 */
function bp_progenitor_groups_customizer_controls( $controls = array() ) {
	return array_merge( $controls, array(
		'group_front_page' => array(
			'label'      => __( 'Enable default front page for groups.', 'buddypress' ),
			'section'    => 'bp_progenitor_group_front_page',
			'settings'   => 'bp_progenitor_appearance[group_front_page]',
			'type'       => 'checkbox',
		),
 ...
 */
function bp_progenitor_group_reset_front_template( $templates = array(), $group = null ) {
	if ( empty( $group->id ) ) {
		return $templates;
	}

	$use_default_front = bp_progenitor_get_appearance_settings( 'group_front_page' );

	// Setting the front template happens too early, so we need this!
	if ( is_customize_preview() ) {
		$use_default_front = bp_progenitor_get_temporary_setting( 'group_front_page', $use_default_front );
	}
	...
	return apply_filters( '_bp_progenitor_group_reset_front_template', $templates );
}
 ...
 */
function bp_progenitor_group_locate_template_part( $template = '' ) {
	$current_group = groups_get_current_group();
	...
	if ( ! isset( $bp_progenitor->groups->current_group_hierarchy ) ) {
		$bp_progenitor->groups->current_group_hierarchy = array(
		...
		if ( bp_groups_get_group_types() ) {
			...
			$bp_progenitor->groups->current_group_hierarchy[] = 'groups/single/%s-group-type-' . sanitize_file_name( $current_group_type )   . '.php';
		}

		$bp_progenitor->groups->current_group_hierarchy = array_merge( $bp_progenitor->groups->current_group_hierarchy, array(
	...
	foreach ( $bp_progenitor->groups->current_group_hierarchy as $part ) {
		$templates[] = sprintf( $part, $template );
	}

	return bp_locate_template( apply_filters( 'bp_progenitor_group_locate_template_part', $templates ), false, true );
}
 ...
 */
function bp_progenitor_group_get_template_part( $template = '' ) {
	$located = bp_progenitor_group_locate_template_part( $template );
 ...
 */
function bp_progenitor_group_is_home_widgets() {
	return true === bp_progenitor()->groups->is_group_home_sidebar;
}
 ...
 */
function bp_progenitor_group_activity_widget_overrides( $args = array() ) {
 ...
 */
function bp_progenitor_group_groups_widget_overrides( $args = array() ) {
	return array_merge( $args, array(
 ...
 */
function bp_progenitor_group_members_widget_overrides( $args = array() ) {
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_add_home_widget_filters() {
	add_filter( 'bp_progenitor_activity_widget_query', 'bp_progenitor_group_activity_widget_overrides', 10, 1 );
	add_filter( 'bp_before_has_groups_parse_args',  'bp_progenitor_group_groups_widget_overrides',   10, 1 );
	add_filter( 'bp_before_has_members_parse_args', 'bp_progenitor_group_members_widget_overrides',  10, 1 );

	do_action( 'bp_progenitor_groups_add_home_widget_filters' );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_remove_home_widget_filters() {
	remove_filter( 'bp_progenitor_activity_widget_query', 'bp_progenitor_group_activity_widget_overrides', 10, 1 );
	remove_filter( 'bp_before_has_groups_parse_args',  'bp_progenitor_group_groups_widget_overrides',   10, 1 );
	remove_filter( 'bp_before_has_members_parse_args', 'bp_progenitor_group_members_widget_overrides',  10, 1 );

	do_action( 'bp_progenitor_groups_remove_home_widget_filters' );
}
 ...
 */
function bp_progenitor_group_get_core_create_screens( $id = '' ) {
 ...
 */
function bp_progenitor_group_get_core_manage_screens( $id = '' ) {
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_notification_filters() {
	$notifications = array(
		array(
