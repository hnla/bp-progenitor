 ...
 * @since 1.0.0
 */
function bp_progenitor_before_groups_directory_content() {
	/**
 ...
 * @since 1.0.0
 */
function bp_progenitor_after_groups_directory_content() {
	/**
 ...
 */
function bp_progenitor_groups_create_hook( $when = '', $suffix = '' ) {
	$hook = array( 'bp' );
 ...
 * @param string $suffix Optional. Use it to add terms at the end of the hook name.
 */
function bp_progenitor_group_hook( $when = '', $suffix = '' ) {
	$hook = array( 'bp' );
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_loop_item() {
	/**
 ...
 * @since 1.0.0
 */
function bp_progenitor_groups_activity_post_form() {
	/**
 ...
 */
function bp_progenitor_group_invites_interface() {
	/**
 ...
 */
function bp_progenitor_groups_get_group_invites_setting() {
	return (int) bp_get_user_meta( bp_displayed_user_id(), '_bp_progenitor_restrict_invites_to_friends' );
}
 ...
 */
function bp_progenitor_group_creation_tabs() {
	$bp = buddypress();
 ...
 * @since 1.0.0
 */
function bp_progenitor_group_creation_screen() {
	return bp_progenitor_group_manage_screen();
}
...
function bp_progenitor_group_manage_screen() {
	...
	if ( ! $is_group_create ) {
		...
		$core_screen = bp_progenitor_group_get_core_manage_screens( $screen_id );

	// It's a group step, get the creation screens.
	} else {
		$core_screen = bp_progenitor_group_get_core_create_screens( $screen_id );
	}
 ...
 * @since 1.0.0
 *
 * @param array $args Optional. See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_group_header_buttons( $args = array() ) {
	$bp_progenitor = bp_progenitor();

	$output = join( ' ', bp_progenitor_get_groups_buttons( $args) );

	// On the group's header we need to reset the group button's global.
	if ( ! empty( $bp_progenitor->groups->group_buttons ) ) {
		unset( $bp_progenitor->groups->group_buttons );
	}
 ...
 * @param array $args Optional. See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_groups_loop_buttons( $args = array() ) {
 ...
 * @param array $args Optional. See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_groups_invite_buttons( $args = array() ) {
	if ( empty( $GLOBALS['groups_template'] ) ) {
		return;
	}
 ...
 * @param array $args Optional. See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_groups_request_buttons( $args = array() ) {
 ...
 * @param array $args Optional. See bp_progenitor_wrapper() for the description of parameters.
 */
function bp_progenitor_groups_manage_members_buttons( $args = array() ) {
	 ...
	 * @param array $args Optional. See bp_progenitor_wrapper() for the description of parameters.
	 */
	function bp_progenitor_get_groups_buttons( $args = array() ) {
		...
		// Membership button on groups loop or single group's header
			 ...
			 */
			add_filter( 'bp_get_group_join_button', 'bp_progenitor_groups_catch_button_args', 100, 1 );

			bp_get_group_join_button( $group );

			remove_filter( 'bp_get_group_join_button', 'bp_progenitor_groups_catch_button_args', 100, 1 );

			if ( ! empty( bp_progenitor()->groups->button_args ) ) {
				$button_args = bp_progenitor()->groups->button_args;
			...
			// Else this is an anchor so use href attr
				...
				unset( bp_progenitor()->groups->button_args );
			}
		}
		 ...
		 */
		$buttons_group = apply_filters( 'bp_progenitor_get_groups_buttons', $buttons, $group, $type );
		...
		if ( ! isset( bp_progenitor()->groups->group_buttons ) || ! is_a( bp_progenitor()->groups->group_buttons, 'BP_Buttons_Group' ) ) {
			$sort = true;
			bp_progenitor()->groups->group_buttons = new BP_Buttons_Group( $buttons_group );

		// It's not the first entry, the order is set, we simply need to update the Buttons Group
		} else {
			$sort = false;
			bp_progenitor()->groups->group_buttons->update( $buttons_group );
		}

		$return = bp_progenitor()->groups->group_buttons->get( $sort );
		 ...
		 * @parem string $type    Whether we're displaying a groups loop or a groups single item.
		 */
		do_action_ref_array( 'bp_progenitor_return_groups_buttons', array( &$return, $group, $type ) );
 ...
 */
function bp_progenitor_group_has_meta() {
	return (bool) bp_progenitor_get_group_meta();
}
 ...
 * @return bool True if the group has meta. False otherwise.
 */
function bp_progenitor_group_has_meta_extra() {
	return (bool) bp_progenitor_get_hooked_group_meta();
}
 ...
 * @return string HTML Output.
 */
function bp_progenitor_group_meta() {
	$meta = bp_progenitor_get_group_meta();
	...
	if ( ! bp_is_group() ) {
		 ...
		 */
		return (object) bp_progenitor_get_group_meta();
	}
}
	 ...
	 */
	function bp_progenitor_get_group_meta() {
		/*
		...
		if ( empty( $group->template_meta ) ) {
			...
			if ( $is_group ) {
				...
				$extra_meta = bp_progenitor_get_hooked_group_meta();
			 ...
			 */
			$group->template_meta = apply_filters( 'bp_progenitor_get_group_meta', $meta, $group, $is_group );
		}
 ...
 * @since 1.0.0
 */
function bp_progenitor_group_template_part() {
	/**
	...
	// Otherwise use BP_progenitor template hierarchy
	} else {
		$template = 'plugins';
		...
		bp_progenitor_group_get_template_part( $template );
	}
 ...
 * @since 1.0.0
 */
function bp_progenitor_group_header_template_part() {
 ...
 * @return string HTML Output
 */
function bp_progenitor_groups_get_customizer_option_link() {
	return bp_progenitor_get_customizer_link( array(
		'object'    => 'group',
		'autofocus' => 'bp_progenitor_group_front_page',
 ...
 * @return string HTML Output
 */
function bp_progenitor_groups_get_customizer_widgets_link() {
	return bp_progenitor_get_customizer_link( array(
		'object'    => 'group',
 ...
 */
function bp_progenitor_group_description_excerpt( $group = null, $length = null ) {
	echo bp_progenitor_get_group_description_excerpt( $group, $length );
}
 ...
 * @return string Excerpt.
 */
function bp_progenitor_get_group_description_excerpt( $group = null, $length = null ) {
	...
	*/
	if ( bp_progenitor_loop_is_grid() && 'groups' === bp_current_component() ) {
		if ( ! $length ) {
			$length = 100;
		} else {
			$length = $length;
	 ...
	 */
	return apply_filters( 'bp_progenitor_get_group_description_excerpt', bp_create_excerpt( $group->description, $length ), $group );
}
