...
bp_progenitor_register_ajax_actions( array(
	array( 'groups_filter'                      => array( 'function' => 'bp_progenitor_ajax_object_template_loader', 'nopriv' => true  ) ),
	array( 'groups_join_group'                  => array( 'function' => 'bp_progenitor_ajax_joinleave_group',        'nopriv' => false ) ),
	array( 'groups_leave_group'                 => array( 'function' => 'bp_progenitor_ajax_joinleave_group',        'nopriv' => false ) ),
	array( 'groups_accept_invite'               => array( 'function' => 'bp_progenitor_ajax_joinleave_group',        'nopriv' => false ) ),
	array( 'groups_reject_invite'               => array( 'function' => 'bp_progenitor_ajax_joinleave_group',        'nopriv' => false ) ),
	array( 'groups_request_membership'          => array( 'function' => 'bp_progenitor_ajax_joinleave_group',        'nopriv' => false ) ),
	array( 'groups_get_group_potential_invites' => array( 'function' => 'bp_progenitor_ajax_get_users_to_invite',    'nopriv' => false ) ),
	array( 'groups_send_group_invites'          => array( 'function' => 'bp_progenitor_ajax_send_group_invites',     'nopriv' => false ) ),
	array( 'groups_delete_group_invite'         => array( 'function' => 'bp_progenitor_ajax_remove_group_invite',    'nopriv' => false ) ),
) );
 ...
 * @return string HTML
 */
function bp_progenitor_ajax_joinleave_group() {
	$response = array(
		'feedback' => sprintf(
 ...
 * @since 1.0.0
 */
function bp_progenitor_ajax_get_users_to_invite() {
	$bp = buddypress();

	$response = array(
		'feedback' => bp_progenitor_message_markup_wrapper( esc_html__( 'There was a problem performing this action. Please try again.', 'buddypress' ), 'error' ),
		'type'     => 'error',
	);
	...
	$message = bp_progenitor_message_markup_wrapper( __( 'You can invite members using the + button, a new nav will appear to let you send your invites', 'buddypress' ), 'info' );

	if ( 'friends' === $request['scope'] ) {
		$request['user_id'] = bp_loggedin_user_id();
		$bp->groups->invites_scope = 'friends';
		$message = bp_progenitor_message_markup_wrapper( __( 'You can invite friends using the + button, a new nav will appear to let you send your invites', 'buddypress' ), 'info' );
	}
	...
	if ( 'invited' === $request['scope'] ) {
		...
		if ( ! bp_group_has_invites( array( 'user_id' => 'any' ) ) ) {
			wp_send_json_error( array(
				'feedback' => bp_progenitor_message_markup_wrapper( __( 'No pending invites found.', 'buddypress' ), 'info' ),
				'type'     => 'info',
			) );
		}
		...
		$message = bp_progenitor_message_markup_wrapper( __( 'You can view all the group\'s pending invites from this screen.', 'buddypress' ), 'info' );
	}

	$potential_invites = bp_progenitor_get_group_potential_invites( $request );

	if ( empty( $potential_invites->users ) ) {
		$error = array(
			'feedback' => bp_progenitor_message_markup_wrapper( __( 'No members were found, try another filter.', 'buddypress' ), 'info' ),
			'type'     => 'info',
		);

		if ( 'friends' === $bp->groups->invites_scope ) {
			$error = array(
				'feedback' => bp_progenitor_message_markup_wrapper( __( 'All your friends are already members of this group or already received an invite to join this group or requested to join it.', 'buddypress' ), 'info' ),
				'type'     => 'info',
			);

			if ( 0 === (int) bp_get_total_friend_count( bp_loggedin_user_id() ) ) {
				$error = array(
					'feedback' => bp_progenitor_message_markup_wrapper( __( 'You have no friends!', 'buddypress' ), 'info' ),
					'type'     => 'info',
				);
			}
	...
	$potential_invites->users = array_map( 'bp_progenitor_prepare_group_potential_invites_for_js', array_values( $potential_invites->users ) );
 ...
 * @since 1.0.0
 */
function bp_progenitor_ajax_send_group_invites() {
	$bp = buddypress();

	$response = array(
		'feedback' =>  bp_progenitor_message_markup_wrapper( __( 'Invites could not be sent, please try again.', 'buddypress' ), 'error' ),
	);
	...
	if ( ! bp_groups_user_can_send_invites( $group_id ) ) {
		wp_send_json_error( array(
			'feedback' => bp_progenitor_message_markup_wrapper( __( 'You are not allowed to send invites for this group.', 'buddypress' ), 'error' ),
			'type'     => 'error',
		) );
	}
	...
	if ( ! empty( $_POST['message'] ) ) {
		...
		add_filter( 'groups_notification_group_invites_message', 'bp_progenitor_groups_invites_custom_message', 10, 1 );
	}
	...
	if ( ! empty( $_POST['message'] ) ) {
		...
		remove_filter( 'groups_notification_group_invites_message', 'bp_progenitor_groups_invites_custom_message', 10, 1 );
	}
	...
	if ( array_search( false, $invited ) ) {
		...
		wp_send_json_error( array(
			'feedback' =>  bp_progenitor_message_markup_wrapper( sprintf( __( 'Invites failed for %d user(s).', 'buddypress' ), count( $errors ) ), 'error' ),
			'users'    => $errors,
			'type'     => 'error',
		) );
	}
	...
	wp_send_json_success( array(
		'feedback' => bp_progenitor_message_markup_wrapper( __( 'Invites sent.', 'buddypress' ), 'success' ),
	) );
}
 ...
 * @since 1.0.0
 */
function bp_progenitor_ajax_remove_group_invite() {
	$user_id  = $_POST['user'];
	$group_id = bp_get_current_group_id();
	...
	if ( empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'groups_invite_uninvite_user' ) ) {
		wp_send_json_error( array(
			'feedback' => bp_progenitor_message_markup_wrapper( __( 'Invites could not be removed, please try again.', 'buddypress' ), 'error' ),
		) );
	}

	if ( BP_Groups_Member::check_for_membership_request( $user_id, $group_id ) ) {
		wp_send_json_error( array(
			'feedback' => bp_progenitor_message_markup_wrapper(  __( 'Too late, the user is now a member of the group.', 'buddypress' ), 'warning' ),
			'code'     => 1,
		) );
	}
	...
	if ( ! groups_uninvite_user( $user_id, $group_id ) ) {
		wp_send_json_error( array(
			'feedback' => bp_progenitor_message_markup_wrapper( __( 'Removing the invite for the user failed.', 'buddypress' ), 'error' ),
			'code'     => 0,
		) );
	}

	wp_send_json_success( array(
		'feedback'    => bp_progenitor_message_markup_wrapper( __( 'No more pending invites for the group.', 'buddypress' ), 'info' ),
