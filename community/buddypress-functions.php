...
/**
 * Loads BuddyPress progenitor Template pack functionality.
 *
 * See @link BP_Theme_Compat() for more.
 *
 * @since 1.0.0
 */
class BP_Progenitor extends BP_Theme_Compat {
	/**
...
//		$this->register_theme();
	...
	/**
	 * BP progenitor register theme package with BP.
	 *
	 * @since 1.0.0
	 */
	...
	/**
	 * BP progenitor global variables.
	 *
	 * @since 1.0.0
	 */
...
//		foreach ( $bp->theme_compat->packages['bp-progenitor'] as $property => $value ) {
//			$this->{$property} = $value;
//		}

		$this->function_prefix = 'bp_progenitor';
		$this->includes_dir  = trailingslashit( get_template_directory() . '/community/includes' );
		$this->lang_dir      = trailingslashit( get_template_directory() . '/community/languages' );
		$this->domain        = 'progenitor';
	...
	protected function includes() {
		...
		do_action_ref_array( 'bp_progenitor_includes', array( &$this ) );
	}
		...
		*/
		...
		bp_set_theme_compat_feature( 'progenitor', array(
			...
			'settings' => array(
				...
				'callback'     => 'bp_progenitor_theme_cover_image',
				'theme_handle' => 'bp-progenitor',
			),
		) );
	}
	...
	protected function setup_actions() {
		...
		// Output progenitor 'forsaken' hook error messages to WP debug log.
//		add_action('bp_init', array( $this, 'progenitor_write_deprecated_hooks_log' ) );
		...
		// Ajax querystring
		add_filter( 'bp_ajax_querystring', 'bp_progenitor_ajax_querystring', 10, 2 );
		...
		add_action( 'widgets_init', 'bp_progenitor_register_sidebars', 11 );

		// Register the Primary Object nav widget
		add_action( 'bp_widgets_init', array( 'BP_progenitor_Object_Nav_Widget', 'register_widget' ) );
		 ...
		 * @since 1.0.0
		 *
		 * @param BP_progenitor $this Current BP_progenitor instance.
		 */
	...
	public function enqueue_styles() {
		...
		$css_dependencies = apply_filters( 'bp_progenitor_css_dependencies', array( 'dashicons' ) );

		$styles = apply_filters( 'bp_progenitor_enqueue_styles', array(
			'bp-progenitor' => array(
	...
	public function register_scripts() {
		...
		$scripts = apply_filters( 'bp_progenitor_register_scripts', array(
			'bp-progenitor' => array(
				'file'         => 'js/buddypress-progenitor%s.js',
		...
		if ( bp_is_active( 'settings' ) || bp_get_signup_allowed() ) {
			$scripts['bp-progenitor-password-verify'] = array(
				'file'         => 'js/password-verify%s.js',
				'dependencies' => array( 'bp-progenitor', 'password-strength-meter' ),
				'footer'       => true,
			);
		}
	...
	public function enqueue_scripts() {
		wp_enqueue_script( 'bp-progenitor' );

		if ( bp_is_register_page() || bp_is_user_settings_general() ) {
			wp_enqueue_script( 'bp-progenitor-password-verify' );
		}
		...
		do_action( 'bp_progenitor_enqueue_scripts' );
	}
	...
	/**
	 * Pass progenitor's hook error messages array to WP debug log function if
	 * 'define( 'WP_DEBUG', true )' & 'define( 'WP_DEBUG_LOG', true )' are set.
	 *
	 *
	 * @since 1.0.0
	 */
	public function progenitor_write_deprecated_hooks_log() {
		...
		if ( $log ) {
			...
			error_log( print_r( '========= progenitor deprecated action hook messages =========', true ) );
		}
	...
	public function localize_scripts() {
		...
		if ( bp_progenitor_is_object_nav_in_sidebar() ) {
			$params['object_nav_parent'] = '.buddypress_object_nav';
		}

		// Set the supported components
		$supported_objects = (array) apply_filters( 'bp_progenitor_supported_components', bp_core_get_packaged_component_ids() );
		$object_nonces     = array();
		...
		foreach ( $supported_objects as $key_object => $object ) {
			...
			$object_nonces[ $object ] = wp_create_nonce( 'bp_progenitor_' . $object );
		}
		...
		if ( is_customize_preview() ) {
			$params['customizer_settings'] = bp_progenitor_get_temporary_setting( 'any' );
		}
		 ...
		 */
		wp_localize_script( 'bp-progenitor', 'BP_Progenitor', apply_filters( 'bp_core_get_js_strings', $params ) );
	}
	...
	public function theme_compat_wrapper( $retval ) {
		...
		return sprintf(
			...
			esc_attr( bp_progenitor_get_container_classes() ),
	...
	public function setup_directory_nav() {
		...
		if ( bp_is_members_directory() ) {
			$nav_items = bp_progenitor_get_members_directory_nav_items();
		} elseif ( bp_is_activity_directory() ) {
			$nav_items = bp_progenitor_get_activity_directory_nav_items();
		} elseif ( bp_is_groups_directory() ) {
			$nav_items = bp_progenitor_get_groups_directory_nav_items();
		} elseif ( bp_is_blogs_directory() ) {
			$nav_items = bp_progenitor_get_blogs_directory_nav_items();
		}
	...
	public function developer_feedbacks() {
		...
		$forsaken_hooks = bp_progenitor_get_forsaken_hooks();
	...
	/**
	 * We'll handle template notices from BP progenitor.
	 *
	 * @since 1.0.0
	 */
...
/**
 * Get a unique instance of BP progenitor
 *
 * @since 1.0.0
 *
 * @return BP_progenitor the main instance of the class
 */
function bp_progenitor() {
	return BP_Progenitor::get_instance();
}
